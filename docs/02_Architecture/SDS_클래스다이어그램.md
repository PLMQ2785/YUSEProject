...
| `OnCollisionStay2D(collision)`                   | 플레이어와 충돌 유지 시 지속적인 데미지를 입힘                           | `void`        | `protected`     |
...
| `OnTriggerEnter2D(other)`     | 플레이어와 충돌 시 데미지를 입히고 투사체를 파괴                     | `void`        | `private`   |
...
### 📦UpgradeData
> **Description:**
> 레벨당 스탯 보너스 값과 레벨에 따른 비용 증가 수식을 포함하여, 영구적인 능력치 업그레이드의 모든 정보를 정의하는 데이터 클래
